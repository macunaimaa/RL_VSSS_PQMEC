...
class Actor_Model:
    def __init__(self, input_shape, action_space, learning_rate, optimizer):
        ...
        X = Dense(256, activation="relu", kernel_initializer=tf.random_normal_initializer(
            stddev=0.001))(X_input)
        # X = Dense(256, activation="relu",
        #           kernel_initializer=tf.random_normal_initializer(stddev=0.001))(X)
        # X = Dense(512, activation="relu",
        #           kernel_initializer=tf.random_normal_initializer(stddev=0.001))(X)
        # X = Dense(512, activation="relu",
        #           kernel_initializer=tf.random_normal_initializer(stddev=0.001))(X)
        X = Dense(128, activation="relu",
                  kernel_initializer=tf.random_normal_initializer(stddev=0.001))(X_input)
        ...
        Adam_changed = tf.keras.optimizers.legacy.Adam(
    ...
    def ppo_loss(self, y_true, y_pred):
        ...
        mu_w1, s_w1 = y_pred[:, 0], tf.math.exp(y_pred[:, 1])
        mu_w2, s_w2 = y_pred[:, 2], tf.math.exp(y_pred[:, 3])

        dist = tfp.distributions.Normal([mu_w1, mu_w2], [s_w1, s_w2])
        y_pred = tf.transpose(dist.log_prob(tf.transpose(actions)))
        ...
        entropy = tf.transpose(dist.entropy())
...
class Critic_Model:
    def __init__(self, input_shape, action_space, learning_rate, optimizer):
        ...
        V = Dense(16, activation="relu", kernel_initializer='he_uniform')(V)
        ...
        self.Critic.compile(loss=[self.critic_PPO2_loss(
            old_values)], optimizer=Adam_changed)
...
class PPOAgent:
    ...
    def __init__(self, env_name):
        ...
        self.Critic = Critic_Model(input_shape=self.state_size, action_space=self.action_size,
                                   learning_rate=self.learning_rate, optimizer=self.optimizer)
    ...
    def act(self, state):  # O problema parece ser essa mudança na funcao act essa aqui
        ...
        mu_w1, s_w1 = action[0], tf.math.exp(action[1])
        mu_w2, s_w2 = action[2], tf.math.exp(action[3])

        # action_w1 = mu_w1 + s_w1 * np.random.randn()  # Amostra da distribuição
        # action_w2 = mu_w2 + s_w2 * np.random.randn()  # Amostra da distribuição
    ...
    def replay(self, states, actions, rewards, predictions, dones, next_states):
        ...
        # print(f'\n\n\na_loss: {a_loss.history["loss"]}\n\n\n')
        # print(f'\n\n\nc_loss: {c_loss.history["loss"]}\n\n\n')
    ...
    def PlotModel(self, score, episode):
        ...
        if self.average_[-1] >= self.max_average:
            ...
            K.set_value(self.Actor.Actor.optimizer.learning_rate,
                        self.learning_rate)
            K.set_value(self.Critic.Critic.optimizer.learning_rate,
                        self.learning_rate)
        else:
            SAVING = ""
